...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
         ...
         */
        final List<Optional<T>> retList = new ArrayList<>();
        list.forEach(elem -> {
            retList.add(Optional.ofNullable(elem).filter(pre));
        });
        return retList;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(elem -> {
            retMap.merge(op.apply(elem), new HashSet<>(Collections.singleton(elem)), (newValue, oldValue) -> {
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((k, v) -> {
            retMap.put(k, map.get(k).orElse(def.get()));
        });
        return retMap;
    }
